...
Комиссия платформы (проверено февраль 2026, реальный тест на заказе #11942504):
- Автор получает 97.5% от ставки (комиссия 2.5%)
- Заказчик платит ×2.03-2.28 от ставки автора (скользящая шкала):
  * 300₽ → ×2.18
  * 1000₽ → ×2.28
  * 3000₽ → ×2.10
  * 10000₽ → ×2.04
...
СТРАТЕГИЯ ЦЕНООБРАЗОВАНИЯ (обновлена 2026-02):
Ставим 60-75% от бюджета (вместо 85-95%) для конкурентоспособности.
При бюджете 3000₽:
  - Конкуренты: 2800₽ → заказчик платит 5880₽
  - Мы: 2000₽ → заказчик платит 4288₽ (на 27% дешевле!)
  - Наш доход: 1950₽, API: 90₽, прибыль: 1860₽ (20.7x ROI)
"""
...
def calculate_price(order: OrderDetail) -> int:
    ...
    UPDATED 2026-02: более агрессивное ценообразование для конкурентоспособности.
    Учитываем что заказчик платит ×2.0-2.3 от ставки автора (проверено реальным тестом).

    Приоритет источников цены:
    1. Бюджет заказчика + средняя ставка (взвешенное среднее × 60-75%)
    2. Только бюджет (60-75% от бюджета, было 85-95%)
    3. Только средняя ставка (65-80% от средней, было 85-98%)
...
def _combined_price(order: OrderDetail) -> Optional[int]:
    ...
    UPDATED 2026-02: более агрессивное ценообразование (60-75% вместо 85-95%)
    для конкурентоспособности с учётом множителя платформы ×2.0-2.3.
    """
    if not order.budget_rub or not order.average_bid:
        return None
    blended = order.budget_rub * 0.6 + order.average_bid * 0.4
    factor = random.uniform(0.60, 0.75)  # Было 0.85-0.95
    return max(MIN_BID, int(blended * factor))


def _try_budget_based(order: OrderDetail) -> Optional[int]:
    """Рассчитать цену на основе бюджета заказчика (60-75%).

    UPDATED 2026-02: снижены коэффициенты для конкурентоспособности.
    При бюджете 3000₽ конкуренты ставят ~2800₽ (заказчик видит 5880₽).
    Мы ставим ~2000₽ (заказчик видит 4288₽) = дешевле на 27%!
    """
...
def _try_average_bid_based(order: OrderDetail) -> Optional[int]:
    """Рассчитать цену на основе средней ставки (65-80%).

    UPDATED 2026-02: более агрессивное ценообразование.
    Ставим существенно ниже средней, чтобы быть конкурентными.
    Заказчик платит ×2.0-2.3, поэтому даже при 70% от средней мы будем
    значительно дешевле для заказчика.
    """
    if not order.average_bid or order.average_bid <= 0:
        return None
    factor = random.uniform(0.65, 0.80)  # Было 0.85-0.98
