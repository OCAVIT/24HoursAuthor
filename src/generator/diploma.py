"""Генератор дипломных работ и ВКР (пошаговая генерация: план → разделы → расширение)."""
...
PLAN_INSTRUCTIONS = (
    "Включи: аннотация (краткое описание работы, цель, методы, результаты — 1 стр.), "
    "введение (актуальность, степень изученности, цель, задачи 4-6 шт., объект, предмет, "
    "методология, научная новизна, практическая значимость, структура), "
    "3-4 главы по 3-4 подраздела в каждой, "
    "заключение (выводы по каждой главе, степень достижения цели, рекомендации), "
    "список литературы (30-50 источников по ГОСТ: учебники, статьи, нормативные документы, "
    "диссертации, интернет-источники, зарубежные). "
...
@dataclass
class DiplomaPlan:
    """Оглавление дипломной работы (для обратной совместимости)."""
    title: str
...
async def generate(
    ...
    """Сгенерировать ВКР/дипломную пошагово: план → разделы → расширение."""
    sw = await stepwise_generate(
        work_type="Дипломная работа (ВКР)",
        title=title,
    ...
    logger.info(
        "ВКР сгенерирована: '%s', ~%d стр., %d токенов, $%.4f",
